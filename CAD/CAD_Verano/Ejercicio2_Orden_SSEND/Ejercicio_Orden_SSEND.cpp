/*	Actividad 2 : Recepcion se realiza en orden con SSEND.
...
int main(int argc, char* argv[]) {
	...
	if (proceso_actual != 0)
	{
		...
		MPI_Ssend(&valor, 1, MPI_INT, destino, flag, MPI_COMM_WORLD);
	...
	else
	{
		...
		{
			MPI_Recv(&valor, 1, MPI_INT, origen, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
			printf("Master. Proceso: %d ha recibido el valor %d del proceso %d.\n", proceso_actual, valor, origen);
		}
