...
void read(char* filename, void* vx, int nx, int ny)
{
	FILE* fp;
	...
	if (NULL == (fp = fopen(filename, "r")))
	{
		fprintf(stderr, "read: no se puede abrir <%s>\n", filename);
		exit(-1);
	}
	...
	{
		fprintf(stderr,
			"read: size mismatch, (nx,ny) = (%d,%d) expected (%d,%d)\n",
			nxt, nyt, nx, ny);
		exit(-1);
	}
...
void write(char* filename, void* vx, int nx, int ny)
{
	FILE* fp;
	...
	{
		fprintf(stderr, "write: no se puede crear <%s>\n", filename);
		exit(-1);
	}

	printf("Escribiendo imagen de %d x %d en: %s\n", nx, ny, filename);
